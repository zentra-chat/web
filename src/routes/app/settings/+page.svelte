<script lang="ts">
	...
	async function handleToggle2FA() {
		if ($currentUser?.twoFactorEnabled) {
			...
			try {
				const { secret } = await api.enable2FA();
	...
	<div class="max-w-7xl mx-auto px-6 py-6 md:px-8 md:py-8">
		...
		<div class="flex gap-6">
			<div class="w-40 space-y-1">
				<button onclick={() => activeTab = 'profile'} class="w-full text-left px-3 py-2 rounded-lg text-sm transition-colors {activeTab === 'profile' ? 'bg-surface-hover text-text-primary' : 'text-text-muted hover:text-text-primary hover:bg-surface'}">Profile</button>
				<button onclick={() => activeTab = 'account'} class="w-full text-left px-3 py-2 rounded-lg text-sm transition-colors {activeTab === 'account' ? 'bg-surface-hover text-text-primary' : 'text-text-muted hover:text-text-primary hover:bg-surface'}">Account</button>
				...
				{:else if activeTab === 'account'}
					<div class="space-y-6">
						<div>
							...
							<Button variant="secondary" onclick={handleLogout} disabled={isLoggingOut}><LogOut size={16} />{isLoggingOut ? 'Logging out...' : 'Log Out'}</Button>
						</div>
						...
						<div>
							...
							<Button variant="secondary" onclick={handleToggle2FA}>{$currentUser?.twoFactorEnabled ? 'Manage 2FA' : 'Enable 2FA'}</Button>
						</div>
									...
									{#each $activeInstanceSavedAccounts as account (account.userId)}
										<div class="flex items-center justify-between p-2 rounded-lg border border-border bg-surface">
											<div class="flex items-center gap-3 min-w-0">
												...
												<div class="min-w-0"><p class="text-sm text-text-primary truncate">{account.displayName}</p><p class="text-xs text-text-muted truncate">@{account.username}</p></div>
											</div>
											<div class="flex items-center gap-2">
												{#if account.userId === $currentUser?.id}
													<span class="text-xs px-2 py-1 rounded bg-primary/20 text-primary">Current</span>
												{:else}
													<button onclick={() => handleSwitchAccount(account.userId)} class="text-xs px-2 py-1 rounded bg-surface-hover text-text-primary hover:bg-surface-active" disabled={switchingAccountId !== null}>{switchingAccountId === account.userId ? 'Switching...' : 'Switch'}</button>
												{/if}
												<button onclick={() => handleRemoveSavedAccount(account.userId)} class="text-xs px-2 py-1 rounded text-text-muted hover:text-text-primary hover:bg-surface-hover" disabled={switchingAccountId !== null}>Remove</button>
											</div>
										</div>
									{/each}
								</div>
					...
					<div class="space-y-6">
						<div>
							...
							<div class="flex gap-2">
								<button class="px-4 py-2 bg-surface border-2 border-primary rounded-lg text-sm text-text-primary">Dark</button>
								<button class="px-4 py-2 bg-surface border border-border rounded-lg text-sm text-text-muted opacity-50 cursor-not-allowed">Light (Coming soon)</button>
							</div>
						</div>
						<div>
							...
							<label class="flex items-center gap-3 cursor-pointer">
								<div class="relative"><input type="checkbox" class="sr-only peer" disabled /><div class="w-10 h-6 bg-surface-hover rounded-full opacity-50"></div><div class="absolute left-1 top-1 w-4 h-4 bg-white rounded-full"></div></div>
						...
						<div>
							...
							<div class="flex flex-wrap gap-2">
								<button onclick={() => handleInstanceSelectorModeChange('disabled')} class="px-4 py-2 rounded-lg text-sm border transition-colors { $instanceSelectorMode === 'disabled' ? 'bg-primary text-background border-primary' : 'bg-surface border-border text-text-muted hover:text-text-primary hover:bg-surface-hover' }" aria-pressed={$instanceSelectorMode === 'disabled'} disabled={isSavingAppearance}>Disabled (default)</button>
								<button onclick={() => handleInstanceSelectorModeChange('auto')} class="px-4 py-2 rounded-lg text-sm border transition-colors { $instanceSelectorMode === 'auto' ? 'bg-primary text-background border-primary' : 'bg-surface border-border text-text-muted hover:text-text-primary hover:bg-surface-hover' }" aria-pressed={$instanceSelectorMode === 'auto'} disabled={isSavingAppearance}>Auto Hide</button>
								<button onclick={() => handleInstanceSelectorModeChange('show')} class="px-4 py-2 rounded-lg text-sm border transition-colors { $instanceSelectorMode === 'show' ? 'bg-primary text-background border-primary' : 'bg-surface border-border text-text-muted hover:text-text-primary hover:bg-surface-hover' }" aria-pressed={$instanceSelectorMode === 'show'} disabled={isSavingAppearance}>Show</button>
							</div>
						</div>
					</div>
				{:else if activeTab === 'developer'}
					<div class="space-y-6">
						<div>
							...
							<div class="flex items-center gap-2 p-2 rounded border border-border bg-surface"><p class="flex-1 text-sm text-text-primary truncate">{$currentUser?.id || 'Unavailable'}</p><Button variant="secondary" size="sm" onclick={handleCopyCurrentUserId}>Copy</Button></div>
						</div>
						<div>
							...
							<label class="flex items-center gap-3 cursor-pointer">
								<div class="relative">
									<input type="checkbox" class="sr-only peer" checked={developerModeEnabled} onchange={(e) => handleDeveloperModeChange((e.target as HTMLInputElement).checked)} disabled={isSavingDeveloperMode} />
						...
						<div class="pt-4 border-t border-border">
							<p class="text-sm text-text-muted">When enabled, you can:<br />• Right-click a server to copy server ID<br />• Right-click a channel to copy channel ID<br />• Use the profile three-dot menu to copy user ID</p>
						</div>
					</div>
